...
- The compile-time error for `consistentRead` on an index query now states the reason. The option's type on GSI queries is a message literal, so the error reads `Type 'true' is not assignable to type '"consistentRead is unavailable on a global secondary index"'` instead of the former `Type 'true' is not assignable to type 'undefined'`. Accepted programs are unchanged.
